...
static int admin_parse_selector(void *ctx, const char *cmd,
  ...
  while (parse_word(&cmd, sizeof(keyword), keyword)) {
    ...
    if (strcmp(keyword, "interface") == 0 || strcmp(keyword, "iface") == 0 ||
...
static void admin_managed_stop(void *ctx, const char *cmd) {
  (void)cmd;
  admin_write(ctx,
              nhrp_stop_managed_hub() ? "Status: ok\n" : "Status: failed\n");
}
...
static void admin_map_add(void *ctx, const char *cmd) {
  ...
  while (parse_word(&cmd, sizeof(word), word)) {
    if (strcmp(word, "interface") == 0 || strcmp(word, "iface") == 0 ||
        strcmp(word, "dev") == 0) {
  ...
  if (nhrp_peer_add_static(iface, &paddr, prefix_length, nbma_ptr, nbma_host,
                           local_nbma_ptr, flags) != NULL) {
    admin_write(ctx, "Status: ok\n");
  } else {
...
static void admin_map_del(void *ctx, const char *cmd) {
  ...
  while (parse_word(&cmd, sizeof(word), word)) {
    if (strcmp(word, "interface") == 0 || strcmp(word, "iface") == 0 ||
      ...
      if (iface == NULL &&
          (iface = nhrp_interface_get_by_name(word, FALSE)) != NULL)
        continue;
      if (pstr[0] == 0)
        snprintf(pstr, sizeof(pstr), "%s", word);
    }
...
static int admin_ha_parse_common(void *ctx, const char *cmd,
  ...
  while (parse_word(&cmd, sizeof(keyword), keyword)) {
    ...
    } else if (strcmp(keyword, "format") == 0 && strcmp(value, "json") == 0 &&
               json != NULL) {
      *json = TRUE;
    } else {
    invalid:
...
static void admin_ha_show(void *ctx, const char *cmd) {
  ...
  if (!admin_ha_parse_common(ctx, cmd, interface_name, sizeof(interface_name),
                             &json))
    return;
  admin_write(ctx, "Status: ok\n\n");
...
static void admin_ha_activate_done(void *ctx, int status, const char *reason,
                                   uint32_t generation) {
  struct admin_remote *remote = ctx;
...
static void admin_ha_mode(void *ctx, const char *cmd) {
  ...
  if (nhrp_ha_set_selection_mode(mode, member, admin_ha_activate_done, remote,
                                 &reason))
    return;
  remote->deferred = FALSE;
  admin_write(ctx, "Status: failed\nReason: %s\n", reason);
...
static void admin_ha_registration_snapshot(void *ctx, const char *cmd) {
  ...
  length = nhrp_ha_hub_snapshot_render(iface, buffer, 65536, offset, limit,
                                       &total, digest);
  admin_write(ctx,
              "Status: ok\nTotal: %zu\nOffset: %zu\nCount: %zu\n"
              "More: %s\nDigest: %s\n\n",
              total, offset,
              total > offset ? (total - offset > limit ? limit : total - offset)
                             : 0,
...
static void admin_ha_registration_sync_apply(void *ctx, const char *cmd) {
  ...
  while (parse_word(&cmd, sizeof(keyword), keyword)) {
    ...
    if (strcmp(keyword, "interface") == 0) {
      ...
      if (!parse_u64(value, &parsed) || parsed == 0 || parsed > UINT16_MAX)
        goto invalid;
...
int admin_init(const char *opennhrp_socket) {
  ...
  if (strlen(opennhrp_socket) >= sizeof(sun.sun_path)) {
    errno = ENAMETOOLONG;
    nhrp_error("Failed initialize admin socket [%s]: %s", opennhrp_socket,
               strerror(errno));
    return 0;
  }
