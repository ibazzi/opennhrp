/* platform/sysdep_netlink.c - Linux netlink glue
 *
...
static int netlink_receive(struct netlink_fd *fd, struct nlmsghdr *reply) {
  ...
  while (!got_reply) {
    ...
    while (NLMSG_OK(h, status)) {
      if (netlink_process_ack(fd, h)) {
      ...
      } else if (reply != NULL && h->nlmsg_seq == reply->nlmsg_seq) {
        len = h->nlmsg_len;
        ...
        nhrp_info("Unknown NLmsg: 0x%08x, len %d", h->nlmsg_type, h->nlmsg_len);
      }
...
static int netlink_send(struct netlink_fd *fd, struct nlmsghdr *req) {
  struct sockaddr_nl nladdr;
  struct iovec iov = {.iov_base = (void *)req, .iov_len = req->nlmsg_len};
  struct msghdr msg = {
      .msg_name = &nladdr,
      .msg_namelen = sizeof(nladdr),
      .msg_iov = &iov,
...
static int netlink_enumerate(struct netlink_fd *fd, int family, int type) {
  ...
  return sendto(fd->fd, (void *)&req, sizeof(req), 0, (struct sockaddr *)&addr,
                sizeof(addr)) >= 0;
}
...
static int netlink_configure_arp(struct nhrp_interface *iface, int pf) {
  ...
  netlink_add_rtattr_l(&req.n, sizeof(req), NDTA_NAME, "arp_cache", 10);

  parms.rta.rta_type = NDTA_PARMS;
  parms.rta.rta_len = RTA_LENGTH(0);
  netlink_add_nested_rtattr_u32(&parms.rta, sizeof(parms), NDTPA_IFINDEX,
                                iface->index);
  netlink_add_nested_rtattr_u32(&parms.rta, sizeof(parms), NDTPA_APP_PROBES, 1);
  netlink_add_nested_rtattr_u32(&parms.rta, sizeof(parms), NDTPA_MCAST_PROBES,
                                0);
  netlink_add_nested_rtattr_u32(&parms.rta, sizeof(parms), NDTPA_UCAST_PROBES,
                                0);

  netlink_add_rtattr_l(&req.n, sizeof(req), NDTA_PARMS, parms.buf,
                       parms.rta.rta_len - RTA_LENGTH(0));
...
static int netlink_link_arp_on(struct nhrp_interface *iface) { return TRUE; }
...
static void netlink_neigh_request(struct nlmsghdr *msg) {
  ...
  nhrp_address_set(&addr, ndm->ndm_family, RTA_PAYLOAD(rta[NDA_DST]),
                   RTA_DATA(rta[NDA_DST]));

  nhrp_debug("NL-ARP(%s) who-has %s", iface->name,
...
static void netlink_neigh_update(struct nlmsghdr *msg) {
  ...
  nhrp_address_set(&sel.protocol_address, ndm->ndm_family,
                   RTA_PAYLOAD(rta[NDA_DST]), RTA_DATA(rta[NDA_DST]));

  if (msg->nlmsg_type == RTM_NEWNEIGH && (ndm->ndm_state & NUD_REACHABLE))
    used = TRUE;

  nhrp_peer_foreach(nhrp_peer_set_used_matching, (void *)(intptr_t)used, &sel);
}
...
static void netlink_link_new(struct nlmsghdr *msg) {
  ...
  if (((ifi->ifi_change & IFF_UP) || (iface->index == 0)) &&
    ...
    nhrp_info("Interface %s: configured UP, mtu=%d", ifname, iface->mtu);
    nhrp_interface_run_script(iface, "interface-up");
  } else {
    nhrp_info("Interface %s: config change, mtu=%d", ifname, iface->mtu);
  }
...
static int netlink_addr_new_nbma(void *ctx, struct nhrp_interface *iface) {
  ...
  if (iface->link_index == ifa->ifa_index) {
    ...
    nhrp_address_set(&iface->nbma_address, ifa->ifa_family,
                     RTA_PAYLOAD(rta[IFA_LOCAL]), RTA_DATA(rta[IFA_LOCAL]));
...
static void netlink_addr_new(struct nlmsghdr *msg) {
  ...
  nhrp_address_set_broadcast(&bcast->protocol_address, ifa->ifa_prefixlen);
...
static void netlink_addr_del(struct nlmsghdr *nlmsg) {
  ...
  nhrp_address_set(&msg.address, ifa->ifa_family, RTA_PAYLOAD(rta[IFA_LOCAL]),
...
static void netlink_route_new(struct nlmsghdr *msg) {
  ...
  iface = nhrp_interface_get_by_index(*(int *)RTA_DATA(rta[RTA_OIF]), FALSE);
  if (iface == NULL)
    return;
  ...
  nhrp_address_set(&peer->protocol_address, rtm->rtm_family,
                   RTA_PAYLOAD(rta[RTA_DST]), RTA_DATA(rta[RTA_DST]));
  if (rta[RTA_GATEWAY] != NULL) {
    nhrp_address_set(&peer->next_hop_address, rtm->rtm_family,
                     RTA_PAYLOAD(rta[RTA_GATEWAY]), RTA_DATA(rta[RTA_GATEWAY]));
  }
...
static void netlink_route_del(struct nlmsghdr *msg) {
  ...
  nhrp_address_set(&sel.protocol_address, rtm->rtm_family,
                   RTA_PAYLOAD(rta[RTA_DST]), RTA_DATA(rta[RTA_DST]));
  if (rta[RTA_GATEWAY] != NULL) {
    nhrp_address_set(&sel.next_hop_address, rtm->rtm_family,
                     RTA_PAYLOAD(rta[RTA_GATEWAY]), RTA_DATA(rta[RTA_GATEWAY]));
  }
...
int kernel_init(void) {
  ...
  ev_timer_init(&neighbor_transaction.timeout, neighbor_transaction_timeout_cb,
                1.0, 0.0);
  ...
  for (i = 0; i < ARRAY_SIZE(netlink_groups); i++) {
    netlink_fds[i].dispatch_size =
        sizeof(route_dispatch) / sizeof(route_dispatch[0]);
    netlink_fds[i].dispatch = route_dispatch;
    if (!netlink_open(&netlink_fds[i], NETLINK_ROUTE, netlink_groups[i]))
      goto err_close_all;
  }
...
int kernel_route(struct nhrp_interface *out_iface, struct nhrp_address *dest,
                 struct nhrp_address *default_source,
                 struct nhrp_address *next_hop, uint16_t *mtu) {
  struct {
    struct nlmsghdr n;
    struct rtmsg r;
    char buf[1024];
  ...
  netlink_add_rtattr_l(&req.n, sizeof(req), RTA_DST, dest->addr,
                       dest->addr_len);
  req.r.rtm_dst_len = dest->addr_len * 8;

  if (default_source != NULL && default_source->type != PF_UNSPEC)
    netlink_add_rtattr_l(&req.n, sizeof(req), RTA_SRC, default_source->addr,
                         default_source->addr_len);
  if (out_iface != NULL)
    netlink_add_rtattr_l(&req.n, sizeof(req), RTA_OIF, &out_iface->index,
                         sizeof(int));
  ...
  if (default_source != NULL && default_source->type == PF_UNSPEC &&
    ...
    nhrp_address_set(default_source, dest->type, RTA_PAYLOAD(rta[RTA_PREFSRC]),
                     RTA_DATA(rta[RTA_PREFSRC]));
  }

  if (next_hop != NULL) {
    if (rta[RTA_GATEWAY] != NULL) {
      nhrp_address_set(next_hop, dest->type, RTA_PAYLOAD(rta[RTA_GATEWAY]),
  ...
  if (mtu != NULL) {
    *mtu = 0;
    ...
    if (rta[RTA_OIF] != NULL) {
      ...
      nbma_iface =
          nhrp_interface_get_by_index(*(int *)RTA_DATA(rta[RTA_OIF]), FALSE);
      if (nbma_iface != NULL)
...
int kernel_send(uint8_t *packet, size_t bytes, struct nhrp_interface *out,
  ...
  struct sockaddr_ll lladdr;
  struct iovec iov = {.iov_base = (void *)packet, .iov_len = bytes};
  struct msghdr msg = {
      .msg_name = &lladdr,
      .msg_namelen = sizeof(lladdr),
      .msg_iov = &iov,
  ...
  if (status < 0) {
    nhrp_error("Cannot send packet to %s(%d): %s", out->name, out->index,
               strerror(errno));
    return FALSE;
  }
...
int kernel_inject_neighbor(struct nhrp_address *neighbor,
  ...
  netlink_add_rtattr_l(&req.n, sizeof(req), NDA_DST, neighbor->addr,
                       neighbor->addr_len);

  if (hwaddr != NULL && hwaddr->type != PF_UNSPEC) {
    req.ndm.ndm_state = NUD_REACHABLE;

    netlink_add_rtattr_l(&req.n, sizeof(req), NDA_LLADDR, hwaddr->addr,
                         hwaddr->addr_len);

    nhrp_debug("NL-ARP(%s) %s is-at %s", dev->name,
               nhrp_address_format(neighbor, sizeof(neigh), neigh),
               nhrp_address_format(hwaddr, sizeof(nbma), nbma));
  } else {
    req.ndm.ndm_state = NUD_FAILED;

    nhrp_debug("NL-ARP(%s) %s not-reachable", dev->name,
               nhrp_address_format(neighbor, sizeof(neigh), neigh));
  }
...
int kernel_inject_neighbor_async(struct nhrp_address *neighbor,
  ...
  if (!netlink_add_rtattr_l(&req.n, sizeof(req), NDA_DST, neighbor->addr,
                            neighbor->addr_len) ||
      !netlink_add_rtattr_l(&req.n, sizeof(req), NDA_LLADDR, hwaddr->addr,
                            hwaddr->addr_len) ||
