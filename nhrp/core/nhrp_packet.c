/* core/nhrp_packet.c - NHRP packet marshalling and tranceiving
 *
...
int nhrp_rate_limit_clear(struct nhrp_address *a, int pref) {
  ...
  for (i = 0; i < RATE_LIMIT_HASH_SIZE; i++) {
    hlist_for_each_entry_safe(rl, c, n, &rate_limit_hash[i], hash_entry) {
      if (a->type == AF_UNSPEC ||
...
static struct nhrp_rate_limit *get_rate_limit(struct nhrp_address *src,
  ...
  if (num_rate_limit_entries == 0) {
    ev_timer_init(&rate_limit_timer, prune_rate_limit_entries_cb,
                  RATE_LIMIT_PURGE_INTERVAL, RATE_LIMIT_PURGE_INTERVAL);
...
void nhrp_buffer_free(struct nhrp_buffer *buffer) { free(buffer); }

struct nhrp_cie *nhrp_cie_alloc(void) {
  return calloc(1, sizeof(struct nhrp_cie));
}

void nhrp_cie_free(struct nhrp_cie *cie) { free(cie); }
...
void nhrp_payload_set_raw(struct nhrp_payload *payload,
...
int nhrp_packet_reroute(struct nhrp_packet *packet,
...
static int nhrp_do_handle_error_indication(struct nhrp_packet *error_pkt,
  ...
  list_for_each_entry(req, &pending_requests, request_list_entry) {
    ...
    if (nhrp_address_cmp(&orig_pkt->src_nbma_address, &req->src_nbma_address))
      continue;
...
} packet_types[] = {[NHRP_PACKET_RESOLUTION_REQUEST] =
                        {
...
static int unmarshall_binary(uint8_t **pdu, size_t *pduleft, size_t size,
                             void *raw) {
  if (*pduleft < size)
    return FALSE;
...
static inline int unmarshall_protocol_address(uint8_t **pdu, size_t *pduleft,
                                              struct nhrp_address *pa) {
...
static inline int unmarshall_nbma_address(uint8_t **pdu, size_t *pduleft,
                                          struct nhrp_address *na) {
  if (*pduleft < na->addr_len + na->subaddr_len)
    return FALSE;

  if (na->addr_len || na->subaddr_len) {
    if (!nhrp_address_set_full(na, na->type, na->addr_len, *pdu,
...
static int unmarshall_cie(uint8_t **pdu, size_t *pduleft, struct nhrp_packet *p,
                          struct nhrp_cie *cie) {
  if (!unmarshall_binary(pdu, pduleft, sizeof(struct nhrp_cie_header),
                         &cie->hdr))
    return FALSE;
...
static int unmarshall_payload(uint8_t **pdu, size_t *pduleft,
                              struct nhrp_packet *packet, int type, size_t size,
                              struct nhrp_payload *p) {
  struct nhrp_cie *cie;
  size_t cieleft;
...
static int unmarshall_packet_header(uint8_t **pdu, size_t *pduleft,
                                    struct nhrp_packet *packet) {
  ...
  packet->src_protocol_address.type =
      nhrp_pf_from_protocol(packet->hdr.protocol_type);
  packet->src_protocol_address.addr_len = phdr->src_protocol_address_len;
  packet->dst_protocol_address.type =
      nhrp_pf_from_protocol(packet->hdr.protocol_type);
  ...
  if (!unmarshall_protocol_address(pdu, pduleft, &packet->src_protocol_address))
    return FALSE;
  return unmarshall_protocol_address(pdu, pduleft,
                                     &packet->dst_protocol_address);
}

static int unmarshall_packet(uint8_t *pdu, size_t pdusize,
                             struct nhrp_packet *packet) {
  ...
  if (!unmarshall_payload(&pos, &pduleft, packet,
                          packet_types[packet->hdr.type].payload_type, size,
                          nhrp_packet_payload(packet, NHRP_PAYLOAD_TYPE_ANY))) {
    nhrp_packet_send_error(packet, NHRP_ERROR_PROTOCOL_ERROR, pos - pdu);
    return FALSE;
  }
  ...
  if (nhrp_extension_wire_validate(pos, pduleft, NHRP_MAX_EXTENSIONS) !=
  ...
  do {
    ...
    if (payload_type == NHRP_PAYLOAD_TYPE_NONE)
      payload_type = NHRP_PAYLOAD_TYPE_RAW;
    if (payload_type == NHRP_PAYLOAD_TYPE_RAW && ntohs(eh.length) == 0)
      payload_type = NHRP_PAYLOAD_TYPE_NONE;
    ...
    if (extension == NULL ||
        !unmarshall_payload(&pos, &pduleft, packet, payload_type,
                            ntohs(eh.length), extension)) {
      nhrp_packet_send_error(packet, NHRP_ERROR_PROTOCOL_ERROR, pos - pdu);
      return FALSE;
    }
  } while (1);
...
static int nhrp_packet_forward(struct nhrp_packet *packet) {
  ...
  nhrp_info(
      "Forwarding packet from nbma src %s, proto src %s to proto dst %s, hop "
      "count %d",
      nhrp_address_format(&packet->src_nbma_address, sizeof(tmp), tmp),
      nhrp_address_format(&packet->src_protocol_address, sizeof(tmp2), tmp2),
      nhrp_address_format(&packet->dst_protocol_address, sizeof(tmp3), tmp3),
...
static int nhrp_packet_receive_local(struct nhrp_packet *packet) {
  ...
  if (packet_types[packet->hdr.type].type == NHRP_TYPE_REPLY) {
    list_for_each_entry(req, &pending_requests, request_list_entry) {
      if (packet->hdr.u.request_id != req->hdr.u.request_id)
        continue;
      if (nhrp_address_cmp(&packet->src_nbma_address, &req->src_nbma_address))
        continue;
    ...
    nhrp_info(
        "Packet type %d from nbma src %s, proto src %s, "
  ...
  if (packet->dst_peer->next_hop_address.type != PF_UNSPEC &&
     ...
     * our local address */
    packet->dst_protocol_address = packet->dst_peer->next_hop_address;
  }
...
int nhrp_packet_receive(uint8_t *pdu, size_t pdulen,
  ...
  if (packet->src_iface->auth_token &&
    ...
    struct nhrp_payload *p;
    p = nhrp_packet_extension(
        packet, NHRP_EXTENSION_AUTHENTICATION | NHRP_EXTENSION_FLAG_NOCREATE,
  ...
  if (peer != NULL && peer->type == NHRP_PEER_TYPE_LOCAL_ADDR)
    ...
    if (!misdirected_registration_warned) {
      nhrp_error(
          "Dropping Registration Request from proto src %s to "
          "non-local proto dst %s on %s; check NHS address "
          "configuration",
          nhrp_address_format(&packet->src_protocol_address, sizeof(tmp), tmp),
          nhrp_address_format(&packet->dst_protocol_address, sizeof(tmp2),
                              tmp2),
          iface->name);
...
static int marshall_binary(uint8_t **pdu, size_t *pduleft, size_t size,
                           void *raw) {
  if (*pduleft < size)
    return FALSE;
...
static inline int marshall_protocol_address(uint8_t **pdu, size_t *pduleft,
                                            struct nhrp_address *pa) {
  if (pa->subaddr_len != 0)
    return FALSE;
...
static inline int marshall_nbma_address(uint8_t **pdu, size_t *pduleft,
                                        struct nhrp_address *na) {
  return marshall_binary(pdu, pduleft, na->addr_len + na->subaddr_len,
                         na->addr);
}
...
static int marshall_payload(uint8_t **pdu, size_t *pduleft,
                            struct nhrp_payload *p) {
  struct nhrp_cie *cie;
...
static int marshall_packet_header(uint8_t **pdu, size_t *pduleft,
                                  struct nhrp_packet *packet) {
...
static int marshall_packet(uint8_t *pdu, size_t pduleft,
                           struct nhrp_packet *packet) {
  uint8_t *pos = pdu;
  struct nhrp_packet_header *phdr = (struct nhrp_packet_header *)pdu;
  ...
  if (!marshall_packet_header(&pos, &pduleft, packet))
    return -1;
  if (!marshall_payload(&pos, &pduleft,
                        nhrp_packet_payload(packet, NHRP_PAYLOAD_TYPE_ANY)))
    return -2;
  ...
  if (packet->hdr.type == NHRP_PACKET_ERROR_INDICATION && pduleft >= 0x10) {
    memset(pos, 0, 0x10);
    pos += 0x10;
    pduleft -= 0x10;
  }
...
int nhrp_packet_route(struct nhrp_packet *packet) {
  ...
  if (packet->dst_peer != NULL) {
    ...
    for (i = 0; i < 4; i++) {
      peer =
          nhrp_peer_route_full(packet->dst_iface, &proto_nexthop, 0,
...
int nhrp_packet_marshall_and_send(struct nhrp_packet *packet) {
  ...
  nhrp_debug(
      "Sending packet %d, from: %s (nbma %s), to: %s (nbma %s)",
      packet->hdr.type,
      nhrp_address_format(&packet->src_protocol_address, sizeof(tmp[0]),
                          tmp[0]),
      nhrp_address_format(&packet->src_nbma_address, sizeof(tmp[1]), tmp[1]),
      nhrp_address_format(&packet->dst_protocol_address, sizeof(tmp[2]),
                          tmp[2]),
...
int nhrp_packet_route_and_send(struct nhrp_packet *packet) {
  ...
  if (packet->dst_peer == NULL || packet->dst_iface == NULL) {
    if (!nhrp_packet_route(packet)) {
      nhrp_packet_send_error(packet, NHRP_ERROR_PROTOCOL_ADDRESS_UNREACHABLE,
                             0);
      return TRUE;
    }
  }
  ...
  payload = nhrp_packet_extension(
      packet, NHRP_EXTENSION_AUTHENTICATION | NHRP_EXTENSION_FLAG_COMPULSORY,
      NHRP_PAYLOAD_TYPE_RAW);
  nhrp_payload_free(payload);
  ...
  if (packet->dst_peer->flags & (NHRP_PEER_FLAG_UP | NHRP_PEER_FLAG_LOWER_UP))
...
int nhrp_packet_send(struct nhrp_packet *packet) {
  ...
  if (packet_types[packet->hdr.type].type != NHRP_TYPE_INDICATION &&
    ...
    if (packet->dst_iface->nat_cie.nbma_address.addr_len && payload != NULL &&
        list_empty(&payload->u.cie_list)) {
      cie = nhrp_cie_alloc();
      if (cie != NULL) {
...
static void nhrp_packet_xmit_timeout_cb(struct ev_timer *w, int revents) {
  struct nhrp_packet *packet = container_of(w, struct nhrp_packet, timeout);

  list_del(&packet->request_list_entry);

  if (packet->dst_peer != NULL && ++packet->retry < packet->max_retries) {
...
int nhrp_packet_send_request_timed(struct nhrp_packet *pkt,
                                   void (*handler)(void *ctx,
                                                   struct nhrp_packet *packet),
                                   void *ctx, ev_tstamp timeout,
                                   int max_retries) {
