/* core/nhrp_peer.c - NHRP peer cache implementation
 *
...
static inline int holding_time_to_expiry_time(int holding_time,
                                              int offset_time) {
...
static char *nhrp_peer_format_full(struct nhrp_peer *peer, size_t len,
  ...
  if (peer->nbma_hostname != NULL) {
    i += snprintf(&buf[i], len - i, " hostname %s", peer->nbma_hostname);
  }
  if (peer->next_hop_nat_oa.type != PF_UNSPEC) {
    i +=
        snprintf(&buf[i], len - i, " nbma-nat-oa %s",
                 nhrp_address_format(&peer->next_hop_nat_oa, sizeof(tmp), tmp));
  }
  if (peer->interface != NULL)
    i += snprintf(&buf[i], len - i, " dev %s", peer->interface->name);
  if (peer->mtu)
  ...
  if (peer->expire_time != 0.0) {
    ...
    if (rel >= 0) {
      i +=
          snprintf(&buf[i], len - i, " expires_in %d:%02d", rel / 60, rel % 60);
    } else {
...
static inline char *nhrp_peer_format(struct nhrp_peer *peer, size_t len,
                                     char *buf) {
...
static void nhrp_peer_resolve_nbma(struct nhrp_peer *peer) {
  ...
  if (peer->interface->nbma_address.type == PF_UNSPEC) {
    r = kernel_route(NULL, &peer->next_hop_address, &peer->my_nbma_address,
                     NULL, &peer->my_nbma_mtu);
    if (!r) {
      nhrp_error(
          "No route to next hop address %s",
          nhrp_address_format(&peer->next_hop_address, sizeof(tmp), tmp));
    }
  } else {
...
char *nhrp_peer_event_reason(union nhrp_peer_event e, int revents,
  ...
  if (revents & EV_CHILD) {
    status = e.child->rstatus;
    if (WIFEXITED(status))
      snprintf(buf, buflen, "exitstatus %d", WEXITSTATUS(status));
    else if (WIFSIGNALED(status))
      snprintf(buf, buflen, "signal %d", WTERMSIG(status));
    else
...
void nhrp_peer_run_script(struct nhrp_peer *peer, char *action,
  ...
  if (peer->my_nbma_address.type != PF_UNSPEC)
    envp[i++] = env("NHRP_SRCNBMA", nhrp_address_format(&peer->my_nbma_address,
                                                        sizeof(tmp), tmp));
  envp[i++] = env("NHRP_DESTADDR", nhrp_address_format(&peer->protocol_address,
                                                       sizeof(tmp), tmp));
  ...
  case NHRP_PEER_TYPE_HA_ACTIVE:
    envp[i++] =
        env("NHRP_DESTNBMA",
            nhrp_address_format(nhrp_peer_active_nbma(peer), sizeof(tmp), tmp));
    if (peer->mtu)
      envp[i++] = envu32("NHRP_DESTMTU", peer->mtu);
    if (peer->next_hop_nat_oa.type != PF_UNSPEC)
      envp[i++] =
          env("NHRP_DESTNBMA_NAT_OA",
              nhrp_address_format(&peer->next_hop_nat_oa, sizeof(tmp), tmp));
    break;
  case NHRP_PEER_TYPE_SHORTCUT_ROUTE:
  case NHRP_PEER_TYPE_LOCAL_ROUTE:
    envp[i++] = env("NHRP_NEXTHOP", nhrp_address_format(&peer->next_hop_address,
                                                        sizeof(tmp), tmp));
    break;
  default:
...
static void nhrp_peer_restart_error(struct nhrp_peer *peer) {
  ...
  case NHRP_PEER_TYPE_DYNAMIC_NHS:
    nhrp_peer_schedule(peer, NHRP_RETRY_ERROR_TIME, nhrp_peer_restart_cb);
    break;
  default:
...
static void nhrp_peer_script_route_up_done(union nhrp_peer_event e,
                                           int revents) {
  ...
  if (nhrp_peer_event_ok(e, revents)) {
    if (revents)
      nhrp_debug(
          "[%s] Route up script: success",
          nhrp_address_format(&peer->protocol_address, sizeof(tmp), tmp));

    peer->flags |= NHRP_PEER_FLAG_UP;
    nhrp_peer_schedule(
        peer, holding_time_to_expiry_time(peer->expire_time - ev_now(), 10),
        nhrp_peer_expire_cb);
  } else {
    nhrp_info("[%s] Route up script: %s; "
              "adding negative cached entry",
              nhrp_address_format(&peer->protocol_address, sizeof(tmp), tmp),
              nhrp_peer_event_reason(e, revents, sizeof(reason), reason));
...
static int nhrp_peer_routes_up(void *ctx, struct nhrp_peer *peer) {
  if (!(peer->flags & NHRP_PEER_FLAG_UP))
    nhrp_peer_run_script(peer, "route-up", nhrp_peer_script_route_up_done);
...
static void nhrp_peer_expire_cb(struct ev_timer *w, int revents) {
  ...
  peer->flags |= NHRP_PEER_FLAG_PRUNE_PENDING;
  nhrp_peer_schedule(peer, peer->expire_time - ev_now(), nhrp_peer_remove_cb);
...
static void nhrp_peer_is_up(struct nhrp_peer *peer) {
  ...
  if (BIT(peer->type) & iface->mcast_mask) {
    ...
    for (i = 0; i < iface->mcast_numaddr; i++) {
      if (!nhrp_address_cmp(&peer->protocol_address, &iface->mcast_addr[i])) {
        mcast = 1;
        break;
      }
    }
  ...
  if (mcast) {
    ...
    if (!was_mcast)
      ...
      nhrp_debug(
          "[%s] Peer remains in multicast list after registration refresh",
          nhrp_address_format(&peer->protocol_address, sizeof(tmp), tmp));
  }
  ...
  if (hlist_hashed(&peer->nbma_hash_entry))
    hlist_del(&peer->nbma_hash_entry);
  if (BIT(peer->type) &
      (BIT(NHRP_PEER_TYPE_CACHED) | BIT(NHRP_PEER_TYPE_DYNAMIC) |
       BIT(NHRP_PEER_TYPE_DYNAMIC_NHS) | BIT(NHRP_PEER_TYPE_STATIC) |
       BIT(NHRP_PEER_TYPE_HA_ACTIVE))) {
    i = nhrp_address_hash(&peer->next_hop_address) %
  ...
  case NHRP_PEER_TYPE_CACHED:
    nhrp_peer_schedule(
        peer, holding_time_to_expiry_time(peer->expire_time - ev_now(), 0),
        nhrp_peer_expire_cb);
    break;
  case NHRP_PEER_TYPE_STATIC:
  case NHRP_PEER_TYPE_DYNAMIC_NHS:
    if (peer->flags & NHRP_PEER_FLAG_REGISTER) {
      nhrp_peer_schedule(peer,
...
static void nhrp_peer_script_peer_up_done(union nhrp_peer_event e,
                                          int revents) {
  struct nhrp_peer *peer = nhrp_peer_from_event(e, revents);
  char tmp[64], reason[32];
  ...
  if (nhrp_peer_event_ok(e, revents)) {
    ...
    kernel_inject_neighbor(&peer->protocol_address, nhrp_peer_active_nbma(peer),
                           peer->interface);
...
static void nhrp_peer_address_query_cb(struct nhrp_address_query *query,
  ...
  if (num_addr > 0) {
    nhrp_info("Resolved '%s' as %s", peer->nbma_hostname,
...
int nhrp_peer_discover_nhs(struct nhrp_peer *peer,
  ...
  if (peer->type != NHRP_PEER_TYPE_DYNAMIC_NHS ||
      !nhrp_address_is_network(&peer->protocol_address, peer->prefix_length)) {
    nhrp_error("Unexpected NHS protocol address change %s -> %s",
               nhrp_address_format(&peer->protocol_address, sizeof(tmp2), tmp2),
...
static void nhrp_peer_handle_registration_reply(void *ctx,
  ...
  if (reply == NULL || reply->hdr.type != NHRP_PACKET_REGISTRATION_REPLY) {
    ...
    if (ec == NHRP_ERROR_HOP_COUNT_EXCEEDED)
      nhrp_peer_discover_nhs(peer, &reply->src_protocol_address);
  ...
  if (ec == NHRP_CODE_SUCCESS && (peer->flags & NHRP_PEER_FLAG_UP) &&
  ...
  if (payload == NULL || cie == NULL) {
    if (peer->interface->nat_cie.nbma_address.addr_len != 0)
      nhrp_info("NAT no longer detected for registration to %s",
                nhrp_address_format(&peer->protocol_address, sizeof(tmp), tmp));
    memset(&peer->interface->nat_cie, 0, sizeof(peer->interface->nat_cie));
  }
...
static void nhrp_peer_send_register_cb(struct ev_timer *w, int revents) {
  ...
  if (peer->interface->protocol_address.type == PF_UNSPEC) {
    ...
    if (peer->interface->protocol_address.type == PF_UNSPEC) {
      nhrp_error("Cannot register to %s: interface %s has no protocol address",
                 nhrp_address_format(&peer->protocol_address, sizeof(dst), dst),
                 peer->interface->name);
      goto error;
    }
  }
  ...
  *cie = (struct nhrp_cie){
  ...
  if (!nhrp_ha_prepare_registration_discovery(peer, packet))
    goto error_free_packet;
  sent = nhrp_packet_send_request(packet, nhrp_peer_handle_registration_reply,
                                  nhrp_peer_get(peer));
...
static int error_on_matching(void *ctx, struct nhrp_peer *peer) { return 1; }
...
static void nhrp_peer_handle_resolution_reply(void *ctx,
  ...
  if (reply == NULL || reply->hdr.type != NHRP_PACKET_RESOLUTION_REPLY) {
    ec = reply ? reply->hdr.u.error.code : -1;
  ...
  nhrp_info("Received Resolution Reply %s/%d is at proto %s nbma %s (code %d)",
            ...
            nhrp_address_format(&cie->protocol_address, sizeof(tmp), tmp),
            nhrp_address_format(&cie->nbma_address, sizeof(nbma), nbma),
            cie->hdr.code);
  ...
  payload = nhrp_packet_extension(
      reply, NHRP_EXTENSION_NAT_ADDRESS | NHRP_EXTENSION_FLAG_NOCREATE,
      NHRP_PAYLOAD_TYPE_CIE_LIST);
  if ((reply->hdr.flags & NHRP_FLAG_RESOLUTION_NAT) && (payload != NULL)) {
    natcie = list_next(&payload->u.cie_list, struct nhrp_cie, cie_list_entry);
    if (natcie != NULL) {
      natoacie = cie;
      nhrp_info(
          "NAT detected: really at proto %s nbma %s",
          nhrp_address_format(&natcie->protocol_address, sizeof(tmp), tmp),
          nhrp_address_format(&natcie->nbma_address, sizeof(nbma), nbma));
    }
  }
  if (natcie == NULL)
    natcie = cie;
  ...
  if (nhrp_address_cmp(&peer->protocol_address, &cie->protocol_address) == 0) {
    ...
    nhrp_address_set_network(&peer->protocol_address, peer->prefix_length);
  ...
  np = nhrp_peer_route(iface, &cie->protocol_address, NHRP_PEER_FIND_EXACT, 0);
  if (np == NULL) {
...
static void nhrp_peer_send_resolve(struct nhrp_peer *peer) {
  ...
  packet->hdr = (struct nhrp_packet_header){
      ...
      .flags = NHRP_FLAG_RESOLUTION_SOURCE_IS_ROUTER |
               NHRP_FLAG_RESOLUTION_AUTHORATIVE | NHRP_FLAG_RESOLUTION_NAT};
  packet->dst_protocol_address = peer->protocol_address;
  ...
  *cie = (struct nhrp_cie){
  ...
  nhrp_packet_extension(
      packet, NHRP_EXTENSION_RESPONDER_ADDRESS | NHRP_EXTENSION_FLAG_COMPULSORY,
      NHRP_PAYLOAD_TYPE_CIE_LIST);
  nhrp_packet_extension(packet, NHRP_EXTENSION_NAT_ADDRESS,
                        NHRP_PAYLOAD_TYPE_CIE_LIST);

  packet->dst_iface = peer->interface;
  nhrp_packet_send_request(packet, nhrp_peer_handle_resolution_reply,
                           nhrp_peer_get(peer));
...
static void nhrp_peer_run_nhs_down(struct nhrp_peer *peer) {
  if ((peer->flags & (NHRP_PEER_FLAG_REGISTER | NHRP_PEER_FLAG_UP |
...
static void nhrp_peer_release(struct nhrp_peer *peer) {
  ...
  case NHRP_PEER_TYPE_HA_ACTIVE:
    ...
    if (peer->protocol_address.type != PF_UNSPEC)
      kernel_inject_neighbor(&peer->protocol_address, NULL, peer->interface);
    break;
...
static void nhrp_peer_dnsmap_query_cb(struct nhrp_address_query *query,
                                      ...
                                      struct nhrp_address *addrs) {
  struct nhrp_peer *np,
      *peer = container_of(query, struct nhrp_peer, address_query);
  struct nhrp_peer_selector sel;
  int i;
  ...
  for (i = 0; i < num_addr; i++) {
    ...
    if (nhrp_peer_foreach(nhrp_peer_renew_nhs_matching, NULL, &sel) != 0)
      continue;
...
static void nhrp_peer_insert_cb(struct ev_timer *w, int revents) {
  ...
  case NHRP_PEER_TYPE_SHORTCUT_ROUTE:
    ...
    else if (nhrp_peer_route(peer->interface, &peer->next_hop_address,
  ...
  case NHRP_PEER_TYPE_NEGATIVE:
    ...
    if (peer->flags & NHRP_PEER_FLAG_UP)
      kernel_inject_neighbor(&peer->protocol_address, NULL, peer->interface);
    nhrp_peer_schedule(peer, NHRP_NEGATIVE_CACHE_TIME, nhrp_peer_remove_cb);
    break;
  default:
    NHRP_BUG_ON("invalid peer type");
    break;
...
static int nhrp_peer_replace_shortcut(void *ctx, struct nhrp_peer *peer) {
  ...
  if (nhrp_address_cmp(&peer->protocol_address, &shortcut->protocol_address) ==
          0 &&
...
void nhrp_peer_insert(struct nhrp_peer *peer) {
  ...
  nhrp_debug("Adding %s %s", nhrp_peer_type[peer->type],
             nhrp_peer_format(peer, sizeof(tmp), tmp));
...
void nhrp_peer_remove(struct nhrp_peer *peer) {
  ...
  nhrp_debug("Removing %s %s", nhrp_peer_type[peer->type],
             nhrp_peer_format(peer, sizeof(tmp), tmp));
...
int nhrp_peer_match(struct nhrp_peer *p, struct nhrp_peer_selector *sel) {
  ...
  if ((sel->flags & NHRP_PEER_FIND_UP) && !(p->flags & NHRP_PEER_FLAG_UP))
    return FALSE;

  if ((sel->flags & NHRP_PEER_FIND_MARK) && !(p->flags & NHRP_PEER_FLAG_MARK))
    return FALSE;

  if (sel->interface != NULL && p->interface != sel->interface &&
      !(p->interface->flags & NHRP_INTERFACE_FLAG_SHORTCUT_DEST))
    return FALSE;

  if (sel->hostname != NULL && (p->nbma_hostname == NULL ||
                                strcmp(sel->hostname, p->nbma_hostname) != 0))
    return FALSE;

  if (sel->parent != NULL && p->parent != sel->parent)
    return FALSE;

  if (sel->local_nbma_address.type != PF_UNSPEC) {
    if (nhrp_address_cmp(&p->my_nbma_address, &sel->local_nbma_address) != 0)
      return FALSE;
  }
  ...
  if (sel->protocol_address.type != PF_UNSPEC) {
    ...
    if (sel->flags & NHRP_PEER_FIND_EXACT) {
      if (nhrp_address_cmp(&p->protocol_address, &sel->protocol_address) != 0)
        return FALSE;
      ...
      if (nhrp_address_prefix_cmp(&p->protocol_address, &sel->protocol_address,
                                  p->prefix_length) != 0)
        return FALSE;
    } else {
      if (p->prefix_length < sel->prefix_length) {
        if (sel->prefix_length == sel->protocol_address.addr_len * 8 &&
            nhrp_address_cmp(&p->protocol_address, &sel->protocol_address) == 0)
          return TRUE;
  ...
  if (sel->next_hop_address.type != PF_UNSPEC) {
    if (nhrp_address_cmp(&p->next_hop_address, &sel->next_hop_address) != 0)
      return FALSE;
  }
...
static int enum_interface_peers(void *ctx, struct nhrp_interface *iface) {
  ...
  return enumerate_peer_cache(&iface->peer_list, ectx->enumerator, ectx->ctx,
                              ectx->sel);
}
...
int nhrp_peer_foreach(nhrp_peer_enumerator e, void *ctx,
   ...
   * other places */
  if (sel != NULL && sel->type_mask == BIT(NHRP_PEER_TYPE_LOCAL_ADDR))
    return 0;
...
static int decide_route(void *ctx, struct nhrp_peer *peer) {
  ...
  if (peer->type != NHRP_PEER_TYPE_SHORTCUT_ROUTE) {
    ...
    if (rd->exclude != NULL &&
        nhrp_address_match_cie_list(&peer->next_hop_address,
                                    &peer->protocol_address, rd->exclude))
      return 0;
  ...
  if (rd->best_found != NULL && rd->found_exact == exact &&
...
struct nhrp_peer *nhrp_peer_route_full(struct nhrp_interface *interface,
                                       struct nhrp_address *dst, int flags,
                                       int type_mask, struct nhrp_address *src,
                                       struct list_head *exclude) {
...
void nhrp_peer_traffic_indication(struct nhrp_interface *iface, uint16_t afnum,
  ...
  peer = nhrp_peer_route(iface, dst, type, ~BIT(NHRP_PEER_TYPE_LOCAL_ROUTE));
  if (peer != NULL)
    return;
...
static int dump_peer(void *ctx, struct nhrp_peer *peer) {
  ...
  nhrp_info("%s %s", nhrp_peer_type[peer->type],
            nhrp_peer_format(peer, sizeof(tmp), tmp));
  (*num_total)++;
  return 0;
}
...
void nhrp_peer_dump_cache(void) {
  ...
  nhrp_info("Total %d peer cache entries, %d allocated entries", num_total,
            nhrp_peer_num_total);
}
...
void nhrp_peer_mark_static(void) {
  ...
  memset(&sel, 0, sizeof(sel));
  sel.type_mask = BIT(NHRP_PEER_TYPE_STATIC) | BIT(NHRP_PEER_TYPE_STATIC_DNS) |
...
struct nhrp_peer *
nhrp_peer_add_static(struct nhrp_interface *iface,
                     struct nhrp_address *proto_addr, uint8_t prefix_length,
                     struct nhrp_address *nbma_addr, const char *nbma_hostname,
                     struct nhrp_address *local_nbma_addr, unsigned int flags) {
  struct nhrp_peer_selector sel;
  struct nhrp_peer *peer = NULL;
  ...
  if (peer != NULL) {
    ...
    peer->afnum = nhrp_afnum_from_pf(peer->next_hop_address.type);
    peer->flags =
        (peer->flags & ~(NHRP_PEER_FLAG_REGISTER | NHRP_PEER_FLAG_CISCO |
  ...
  peer->prefix_length =
      prefix_length ? prefix_length : (proto_addr->addr_len * 8);
