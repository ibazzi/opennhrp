/* core/nhrp_server.c - NHRP request handling
 *
...
static int nhrp_server_is_ha_registration(struct nhrp_packet *packet) {
  return packet->src_iface != NULL && packet->src_iface->ha_member_id[0] != 0 &&
         nhrp_packet_extension(packet,
                               NHRP_EXTENSION_HA | NHRP_EXTENSION_FLAG_NOCREATE,
...
static struct nhrp_peer *ha_probe_reply_peer(struct nhrp_packet *packet) {
  ...
  peer->flags =
      NHRP_PEER_FLAG_UP | NHRP_PEER_FLAG_LOWER_UP | NHRP_PEER_FLAG_REPLACED;
  ...
  if (packet->src_iface->nbma_address.type != PF_UNSPEC) {
    ...
    peer->my_nbma_mtu = packet->src_iface->nbma_mtu;
                           ...
                           &peer->my_nbma_address, NULL, &peer->my_nbma_mtu)) {
    nhrp_peer_put(peer);
    return NULL;
  }
  return peer;
...
static int nhrp_handle_resolution_request(struct nhrp_packet *packet) {
  ...
  if (ha_probe != 2)
    nhrp_info(
        "Received Resolution Request from proto src %s to %s",
    ...
    nhrp_debug(
        "Received HA Resolution Probe from proto src %s to %s",
        nhrp_address_format(&packet->src_protocol_address, sizeof(tmp), tmp),
        nhrp_address_format(&packet->dst_protocol_address, sizeof(tmp2), tmp2));
  ...
  packet->hdr.flags &= NHRP_FLAG_RESOLUTION_SOURCE_IS_ROUTER |
                       ...
                       NHRP_FLAG_RESOLUTION_UNIQUE | NHRP_FLAG_RESOLUTION_NAT;
  ...
  if (ha_probe != 2)
    nhrp_info(
        "Sending Resolution Reply %s/%d is-at %s (holdtime %d)",
        nhrp_address_format(&packet->dst_protocol_address, sizeof(tmp), tmp),
        cie->hdr.prefix_length,
        nhrp_address_format(&cie->nbma_address, sizeof(tmp2), tmp2),
        ntohs(cie->hdr.holding_time));
  else
    nhrp_debug(
        "Sending HA Resolution Probe Reply %s/%d is-at %s",
        nhrp_address_format(&packet->dst_protocol_address, sizeof(tmp), tmp),
        cie->hdr.prefix_length,
        nhrp_address_format(&cie->nbma_address, sizeof(tmp2), tmp2));
  ...
  payload = nhrp_packet_extension(
      packet, NHRP_EXTENSION_NAT_ADDRESS | NHRP_EXTENSION_FLAG_NOCREATE,
...
static int find_one(void *ctx, struct nhrp_peer *p) { return 1; }
...
static int remove_old_registrations(void *ctx, struct nhrp_peer *p) {
  ...
  if (nhrp_address_cmp(&peer->protocol_address, &p->protocol_address) == 0 &&
    ...
    peer->flags |= p->flags & (NHRP_PEER_FLAG_UP | NHRP_PEER_FLAG_LOWER_UP);
...
static void nhrp_server_finish_reg(struct nhrp_pending_request *pr) {
  ...
  if (pr->rpeer != NULL && nhrp_packet_reroute(packet, pr->rpeer)) {
    if (ha_registration && pr->num_error == 0)
      nhrp_info(
          "Sending HA Registration Reply from proto src %s to %s (%d bindings "
          "accepted, 0 rejected)",
          nhrp_address_format(&packet->dst_protocol_address, sizeof(tmp), tmp),
          nhrp_address_format(&packet->src_protocol_address, sizeof(tmp2),
                              tmp2),
          pr->num_ok);
    else
      nhrp_info(
          "Sending Registration Reply from proto src %s to %s (%d bindings "
          "accepted, %d rejected)",
          nhrp_address_format(&packet->dst_protocol_address, sizeof(tmp), tmp),
          nhrp_address_format(&packet->src_protocol_address, sizeof(tmp2),
                              tmp2),
    ...
    nhrp_info(
        "Dropping Registration Reply from proto src %s to %s",
        nhrp_address_format(&packet->dst_protocol_address, sizeof(tmp), tmp),
...
static void nhrp_server_finish_cie_reg_cb(union nhrp_peer_event e,
  ...
  if (revents != 0 && nhrp_peer_event_ok(e, revents)) {
    ...
    if (revents == 0)
      ...
      nhrp_error("[%s] Peer registration failed: %s", tmp,
...
static void nhrp_server_start_cie_reg(struct nhrp_pending_request *pr) {
  ...
  if (peer == NULL) {
    ...
    for (; cie->cie_list_entry.next != &pr->payload->u.cie_list;
         cie =
             list_next(&cie->cie_list_entry, struct nhrp_cie, cie_list_entry)) {
      pr->num_error++;
      cie->hdr.code = NHRP_CODE_INSUFFICIENT_RESOURCES;
    }
    pr->num_error++;
  ...
  memset(&sel, 0, sizeof(sel));
  sel.flags = NHRP_PEER_FIND_EXACT;
  sel.type_mask = NHRP_PEER_TYPEMASK_ALL & ~(NHRP_PEER_TYPEMASK_REMOVABLE |
  ...
  if (nhrp_peer_foreach(find_one, peer, &sel) != 0) {
    ...
    nhrp_peer_run_script(peer, "peer-register", nhrp_server_finish_cie_reg_cb);
  }
}
...
static int nhrp_handle_registration_request(struct nhrp_packet *packet) {
  ...
  if (ha_registration)
    nhrp_info(
        "Received HA Registration Request from proto src %s to %s",
        nhrp_address_format(&packet->src_protocol_address, sizeof(tmp), tmp),
        nhrp_address_format(&packet->dst_protocol_address, sizeof(tmp2), tmp2));
  else
    nhrp_info(
  ...
  payload = nhrp_packet_extension(
      packet, NHRP_EXTENSION_FORWARD_TRANSIT_NHS | NHRP_EXTENSION_FLAG_NOCREATE,
  ...
  packet->hdr.hop_count = NHRP_PACKET_DEFAULT_HOP_COUNT;
  packet->hdr.flags &=
      NHRP_FLAG_REGISTRATION_UNIQUE | NHRP_FLAG_REGISTRATION_NAT;
...
static int remove_peer_by_nbma(void *ctx, struct nhrp_peer *peer) {
  ...
  if (!nhrp_address_is_any_addr(nbma)) {
    if (peer->type == NHRP_PEER_TYPE_SHORTCUT_ROUTE) {
      ...
      nexthop =
          nhrp_peer_route(peer->interface, &peer->next_hop_address,
                          NHRP_PEER_FIND_EXACT, NHRP_PEER_TYPEMASK_ADJACENT);
      if (nexthop != NULL)
  ...
  if (peer_nbma != NULL && nhrp_address_cmp(peer_nbma, nbma) == 0)
...
static int nhrp_handle_purge_request(struct nhrp_packet *packet) {
  ...
  nhrp_info(
      "Received Purge Request from proto src %s to %s",
      nhrp_address_format(&packet->src_protocol_address, sizeof(tmp), tmp),
      nhrp_address_format(&packet->dst_protocol_address, sizeof(tmp2), tmp2));
  ...
  list_for_each_entry(cie, &payload->u.cie_list, cie_list_entry) {
    ...
    nhrp_peer_foreach(remove_peer_by_nbma, &cie->nbma_address, &sel);
    nhrp_rate_limit_clear(&cie->protocol_address, cie->hdr.prefix_length);
  }
...
static int nhrp_handle_traffic_indication(struct nhrp_packet *packet) {
  ...
  if (!nhrp_address_parse_packet(packet->hdr.protocol_type, pl->u.raw->length,
                                 pl->u.raw->data, NULL, &dst))
    return FALSE;

  /* Shortcuts enabled? */
  if (packet->src_iface->flags & NHRP_INTERFACE_FLAG_SHORTCUT) {
    nhrp_info(
        "Traffic Indication from proto src %s; "
        "about packet to %s",
        nhrp_address_format(&packet->src_protocol_address, sizeof(tmp), tmp),
        nhrp_address_format(&dst, sizeof(tmp2), tmp2));

    nhrp_peer_traffic_indication(packet->src_iface, packet->hdr.afnum, &dst);
  } else {
    nhrp_info(
        "Traffic Indication ignored from proto src %s; "
        "about packet to %s",
        nhrp_address_format(&packet->src_protocol_address, sizeof(tmp), tmp),
        nhrp_address_format(&dst, sizeof(tmp2), tmp2));
  }
