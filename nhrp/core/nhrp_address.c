/* core/nhrp_address.c - NHRP address conversion functions
 *
...
static void ares_timeout_cb(struct ev_timer *w, int revents) {
  struct nhrp_resolver *r = container_of(w, struct nhrp_resolver, timeout);

  ares_process(r->channel, NULL, NULL);
}

static void ares_prepare_cb(struct ev_prepare *w, int revents) {
  struct nhrp_resolver *r = container_of(w, struct nhrp_resolver, prepare);
...
int nhrp_address_parse(const char *string, struct nhrp_address *addr,
  ...
  r = sscanf(string, "%hhd.%hhd.%hhd.%hhd/%hhd", &addr->addr[0], &addr->addr[1],
             &addr->addr[2], &addr->addr[3], prefix_len ? prefix_len : &tmp);
...
#endif
{
  struct nhrp_address_query *query = (struct nhrp_address_query *)arg;
  struct nhrp_address addr[16];
  int i;

  if (status == ARES_SUCCESS) {
    for (i = 0; he->h_addr_list[i] != NULL && i < ARRAY_SIZE(addr); i++)
...
void nhrp_address_resolve(struct nhrp_address_query *query,
  ...
  ares_gethostbyname(resolver.channel, hostname, AF_INET, ares_address_cb,
                     query);
}
...
int nhrp_address_set(struct nhrp_address *addr, uint16_t type, uint8_t len,
                     uint8_t *bytes) {
...
int nhrp_address_set_full(struct nhrp_address *addr, uint16_t type, uint8_t len,
                          uint8_t *bytes, uint8_t sublen, uint8_t *subbytes) {
...
int nhrp_address_cmp(const struct nhrp_address *a,
...
const char *nhrp_address_format(const struct nhrp_address *addr, size_t buflen,
                                char *buffer) {
  switch (addr->type) {
  case PF_UNSPEC:
    ...
    break;
  case PF_INET:
    snprintf(buffer, buflen, "%d.%d.%d.%d", addr->addr[0], addr->addr[1],
             addr->addr[2], addr->addr[3]);
    break;
  default:
    snprintf(buffer, buflen, "(proto 0x%04x)", addr->type);
    break;
  }
