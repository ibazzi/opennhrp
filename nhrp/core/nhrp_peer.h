/* core/nhrp_peer.h - NHRP peer cache definitions
 *
...
#define NHRP_PEER_TYPE_SHORTCUT_ROUTE                                          \
  0x03                              /* Received/relayed resolution for route */
...
#define NHRP_PEER_TYPE_LOCAL_ROUTE                                             \
  0x08 /* Non-local destination, with local route */
#define NHRP_PEER_TYPE_LOCAL_ADDR                                              \
  0x09 /* Local destination (IP or off-NBMA subnet) */
...
#define NHRP_PEER_TYPEMASK_ADJACENT                                            \
  (BIT(NHRP_PEER_TYPE_CACHED) | BIT(NHRP_PEER_TYPE_DYNAMIC) |                  \
   ...
   BIT(NHRP_PEER_TYPE_HA_ACTIVE) | BIT(NHRP_PEER_TYPE_LOCAL_ADDR))

#define NHRP_PEER_TYPEMASK_REMOVABLE                                           \
  (BIT(NHRP_PEER_TYPE_INCOMPLETE) | BIT(NHRP_PEER_TYPE_NEGATIVE) |             \
   BIT(NHRP_PEER_TYPE_CACHED) | BIT(NHRP_PEER_TYPE_SHORTCUT_ROUTE) |           \
   BIT(NHRP_PEER_TYPE_DYNAMIC))

#define NHRP_PEER_TYPEMASK_PURGEABLE                                           \
  (NHRP_PEER_TYPEMASK_REMOVABLE | BIT(NHRP_PEER_TYPE_DYNAMIC_NHS) |            \
   BIT(NHRP_PEER_TYPE_STATIC) | BIT(NHRP_PEER_TYPE_STATIC_DNS))

#define NHRP_PEER_TYPEMASK_ALL                                                 \
  (NHRP_PEER_TYPEMASK_PURGEABLE | BIT(NHRP_PEER_TYPE_LOCAL_ROUTE) |            \
   BIT(NHRP_PEER_TYPE_LOCAL_ADDR) | BIT(NHRP_PEER_TYPE_HA_ACTIVE))

/* For routing via NHS */
#define NHRP_PEER_TYPEMASK_ROUTE_VIA_NHS                                       \
  (BIT(NHRP_PEER_TYPE_DYNAMIC) | BIT(NHRP_PEER_TYPE_DYNAMIC_NHS) |             \
   BIT(NHRP_PEER_TYPE_STATIC) | BIT(NHRP_PEER_TYPE_HA_ACTIVE) |                \
   BIT(NHRP_PEER_TYPE_LOCAL_ROUTE) | BIT(NHRP_PEER_TYPE_LOCAL_ADDR))
...
#define NHRP_PEER_FLAG_REMOVED                                                 \
  0x100                           /* Deleted, but not removed from cache yet */
#define NHRP_PEER_FLAG_MARK 0x200 /* Can be used to temporarily mark peers */
#define NHRP_PEER_FLAG_CONFIGURED                                              \
  0x400 /* Explicitly configured in config file */
...
struct nhrp_peer *nhrp_peer_find_by_nbma(struct nhrp_interface *iface,
...
static inline struct nhrp_address *
nhrp_peer_active_nbma(struct nhrp_peer *peer) {
