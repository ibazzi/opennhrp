...
static int nhrp_peer_cache_write(void *ctx, struct nhrp_peer *peer)
{
	...
	if (fprintf(save->file, "%s %s %s/%u %s %s %u %" PRId64 "\n",
		    ...
		    peer->prefix_length,
		    nhrp_address_format(&peer->next_hop_address,
					sizeof(next_hop), next_hop),
		    peer->next_hop_nat_oa.type == PF_UNSPEC ? "-" :
			nhrp_address_format(&peer->next_hop_nat_oa,
					    sizeof(nat_oa), nat_oa),
...
static int nhrp_peer_cache_path(const char *directory, char *path, size_t size)
{
	int length;

	if (directory == NULL)
		return FALSE;
	length = snprintf(path, size, "%s/%s", directory,
			  NHRP_PEER_CACHE_FILE);
					 ...
					 struct nhrp_address *address)
{
	...
	if (inet_pton(AF_INET, text, &ipv4) != 1)
		return FALSE;
	return nhrp_address_set(address, PF_INET, sizeof(ipv4),
				(uint8_t *)&ipv4);
}
					...
					struct nhrp_peer_cache_record *record)
{
	...
	if (sscanf(line, "%15s %15s %31s %31s %31s %15s %31s %c",
		   type, interface, protocol, next_hop, nat_oa, mtu_text,
		   expiry_text, &extra) != 7 ||
	    !nhrp_peer_cache_parse_uint(mtu_text, UINT16_MAX, &mtu) ||
	    !nhrp_peer_cache_parse_uint(expiry_text, INT64_MAX, &expiry) ||
	    !nhrp_peer_cache_parse_protocol(protocol,
					    &record->protocol_address,
					    &record->prefix_length) ||
	    !nhrp_peer_cache_parse_address(next_hop,
	...
	else if (!nhrp_peer_cache_parse_address(nat_oa,
						&record->next_hop_nat_oa))
		return FALSE;
