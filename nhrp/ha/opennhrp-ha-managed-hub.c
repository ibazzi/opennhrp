/* ha/opennhrp-ha-managed-hub.c - Managed multi-Hub HA coordinator */
...
static int set_core_state(struct managed_runtime *runtime) {
  ...
  for (attempt = 0; attempt < 2; attempt++) {
    ...
    int serviceable =
        ...
        (service_quorum(runtime, monotonic_ms()) || failback_handoff);
    const char *role =
        serviceable ? (local_leader ? "leader" : "follower") : "standby";
    int status;
...
static int snapshot_apply(struct managed_runtime *runtime,
  ...
  for (line = strtok_r(copy, "\n", &save); line != NULL;
    ...
    if (++count > 4096 ||
        sscanf(
            line, "entry %63s %u %63s %63s %u %u %u %llu %llu %u %63s %15s %c",
            protocol, &prefix, nbma, nat_oa, &mtu, &holding, &flags, &old_term,
            &old_index, &registration_id, owner_member, origin, &extra) != 12 ||
  ...
  for (line = strtok_r(copy, "\n", &save); line != NULL;
    ...
    if (sscanf(line, "entry %63s %u %63s %63s %u %u %u %llu %llu %u %63s %15s",
    ...
    snprintf(command, sizeof(command),
             ...
             holding, flags, registration_id, owner_member, old_term,
             old_index);
    if (!admin_ok(runtime, command))
      goto failed;
  }
