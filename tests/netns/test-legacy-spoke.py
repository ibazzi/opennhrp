...
def send_registration(hub, mode, addresses, source="192.0.2.13"):
    member = b"bootstrap"
    ...
    ip = socket.inet_aton
    body = ip(source) + ip(addresses[0]) + ip("10.20.0.1")
    for address in addresses:
        body += struct.pack("!BBHHHBBBB", 0, 32, 0, 1400, 120, 4, 0, 4, 0)
        body += ip(source) + ip(address)
...
def main():
    ...
    with tempfile.TemporaryDirectory(prefix="opennhrp-legacy-") as directory:
        ...
        try:
            ...
            register("h1", "legacy", "10.20.0.200")
            ns("sp", sys.executable, __file__, "--send-source", "192.0.2.11",
               "legacy", "192.0.2.14", "10.20.0.200")
            assert "NBMA-Address: 192.0.2.14" in ctl("h1", "show protocol", "10.20.0.200")
            ...
            for term in (1002, 1003, 1004):
                ...
                for last in (200, 201, 203, 204, 206):
                    ping("h1", f"10.20.0.{last}")
            print("PASS: NBMA replacement, legacy/HA/Vendor/invalid/multiple CIE, renewal, sync, failover, failback, neighbors and GRE ping")
        except Exception:
...
if __name__ == "__main__":
    ...
    elif len(sys.argv) > 1 and sys.argv[1] == "--send-source":
        send_registration(sys.argv[2], sys.argv[3], sys.argv[5:], sys.argv[4])
    else:
        main()
